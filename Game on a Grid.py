# -*- coding: utf-8 -*-
"""
Created on Fri Aug  6 18:08:05 2021
...
for y in range(h):
    for x in range(w):
        ans += P[y][x]
        grid_num = grid_to_num(x, y)
        for a, b in dxdy:
            nexty, nextx = y+a, x+b
            if valid(nextx, nexty):
                next_grid_num = grid_to_num(nextx, nexty)
                cost = - (P[y][x] * P[nexty][nextx])
                edge.append((cost, grid_num, next_grid_num))
